...
async def list_work_categories() -> list[dict[str, str]]:
...
async def list_public_works(service: WorkService = Depends(get_work_service)) -> list[WorkRead]:
    return await service.list_public_works()
...
async def submit_work(
    ...
    return await service.submit_work(payload)


@router.get("/moderation/pending", response_model=list[WorkRead])
async def list_pending_works(service: WorkService = Depends(get_work_service)) -> list[WorkRead]:
    return await service.list_pending_works()
...
async def get_public_work(
    ...
    return await service.get_public_work(work_id)
...
async def approve_work(
    ...
    return await service.approve_work(work_id)
...
async def reject_work(
    ...
    return await service.reject_work(work_id)
