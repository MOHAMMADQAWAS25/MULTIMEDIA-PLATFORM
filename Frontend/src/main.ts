...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const text = errorPayload.error?.message ?? formatApiDetail(errorPayload.detail) ?? 'Request failed.';
...
function formatApiDetail(detail: unknown): string | null {
  ...
  const messages = detail
    .map((item: unknown) => {
      if (!isValidationErrorItem(item) || item.msg === undefined) {
        return null;
      }

      const fieldPath = item.loc?.filter((part) => part !== 'body').join('.');
